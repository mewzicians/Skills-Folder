---
name: game-skill-creator
description: Create or update effective Codex skills for game design, engineering, debugging, balance, UX, playtesting, audio, content, production, and release workflows. Use when building a reusable game-development skill or specializing one for a particular game.
---

# Game Skill Creator

> Adapted from OpenAI's Codex skill-creator workflow for reusable and
> project-specialized game development. This file has been modified.

Start from a concrete game-development outcome and preserve a clean boundary
between reusable craft, engine-specific procedure, and one project's rules.
For reusable skills, route project assumptions through a documented project
profile or focused references. For specialized skills, keep the generic
mission intact and add only the game's source paths, terminology, invariants,
tests, and release gates.

When a reusable skill suite needs an explicit specialization layer, copy
`assets/GAME_PROJECT_PROFILE.template.md` into the target repository as
`GAME_PROJECT_PROFILE.md` and fill it with that game's authoritative paths,
status vocabulary, constraints, tests, mirrors, and release gates.
